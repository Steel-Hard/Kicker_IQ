...
from app.services.analytics import sugerir_substitutos, construir_perfis
...
async def get_similaridade(athlete_id: str, top_n: int = 3, current_user: str = Depends(get_current_user)):
    ...
    # precisamos reconstruir o dataframe com z_scores flats
    ...
    for s in sessoes:
        rec = s.model_dump()
        for col in z_cols:
            f_name = col.replace("Z_", "")
            rec[col] = rec.get("z_scores", {}).get(f_name, 0.0)
        rec['Athlete ID'] = rec['athlete_id']
        records.append(rec)
        
    df_z = pd.DataFrame(records)
    perfis = construir_perfis(df_z, z_cols)
