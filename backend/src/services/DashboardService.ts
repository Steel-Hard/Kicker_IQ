...
export class DashboardService {
  async getDashboardSummary() {
    ...
    const radarTotal = athletes.reduce(
      (acc, a) => ({
    ...
    const topPerformers = [...athletes]
      .map((a) => {
        const score =
          (a.radar.velocidade + a.radar.explosividade + a.radar.resistencia) /
          3;
    ...
    const teamStats = {
      ...
      avgSprintDist: Math.round(
        athletes.reduce((acc, a) => acc + a.sprintDistance, 0) / count,
      ),
      avgLoad: Math.round(
        athletes.reduce((acc, a) => acc + a.weeklyLoad, 0) / count,
      ),
      ...
      sprintHistory: loadEvolution.map((e) => ({
        name: e.jornada,
        time: e.carga,
      })),
    };
